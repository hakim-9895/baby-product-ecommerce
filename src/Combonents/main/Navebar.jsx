...
const  {setSearch,login,setLogin, handlLogin,handlLogout,cart}= useContext(Productcontext)
  ...
  <button onClick={toggleModal} className="relative">
    <CiShoppingCart className="text-custom-teal text-3xl" />
    {/* Badge for Cart Length */}
    {cart.length > 0 && (
      <span className="absolute -top-1 -right-1 text-xs bg-green-600 text-white rounded-full px-2 py-1">
        {cart.length}
      </span>
    )}
  </button>
